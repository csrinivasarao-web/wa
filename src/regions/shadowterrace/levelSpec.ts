...
export function paramsForChapter(chapter: number, seed: string, levelInChapter: number, ultra = false): ShadowParams {
  ...
  if (ultra) return { size: 5, maxHeight: 4, density: [0.55, 0.75], footprint: true, count: 'minimum', fixedStones: [1, 2], minStones: 14 };
  const late = levelInChapter >= 1;
  switch (chapter) {
    case 0:
      return { size: 3, maxHeight: 2, density: [0.6, 0.9], footprint: false, count: 'none', fixedStones: [0, 0], minStones: 4 };
    case 1:
      return { size: late ? 4 : 3, maxHeight: 3, density: [0.55, 0.8], footprint: true, count: 'none', fixedStones: [0, 0], minStones: 6 };
    case 2:
      return { size: 4, maxHeight: 3, density: [0.5, 0.75], footprint: true, count: 'exact', fixedStones: [0, 0], minStones: 8 };
    default:
      return { size: rng.chance(0.5) ? 4 : 5, maxHeight: 4, density: [0.5, 0.7], footprint: true, count: late ? 'minimum' : 'exact', fixedStones: [1, 2], minStones: 12 };
  }
}

// Handcrafted terraces from a drawn heightmap (rows top to bottom).
function terrace(seed: string, chapter: number, rows: number[][], options: { footprint?: boolean; count?: 'exact' | 'minimum'; fixed?: Array<[number, number]> } = {}): ShadowLevel {
  const n = rows.length;
  ...
  const level: ShadowLevel = {
    ...
    footprint: options.footprint ? heights.map((h) => h > 0) : null,
...
export function handcraftedLevels(): Record<number, () => ShadowLevel> {
  return {
    ...
    // A ziggurat with its moonlit floor plan.
    5: () =>
      terrace(
    ...
    // A small castle: four towers, walls between, an empty court, counted exactly.
    7: () =>
      terrace(
